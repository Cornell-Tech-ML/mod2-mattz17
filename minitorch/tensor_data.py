...
def broadcast_index(
    ...
    for i in range(1, small_dims + 1):
        big_idx_pos = big_dims - i
        small_idx_pos = small_dims - i

        if shape[small_idx_pos] == 1:
            out_index[small_idx_pos] = 0
        else:
            out_index[small_idx_pos] = big_index[big_idx_pos]
...
class TensorData:
    ...
    def shape_broadcast(shape_a: UserShape, shape_b: UserShape) -> UserShape:
        ...
        Arguments:
        ---------
            shape_a: The first shape to be broadcasted.
            shape_b: The second shape to be broadcasted.

        Returns:
        -------
            UserShape: The resulting shape after broadcasting.

        """
        return shape_broadcast(shape_a, shape_b)

    def index(self, index: Union[int, UserIndex]) -> int:
        """Converts a user index to a position in storage.
